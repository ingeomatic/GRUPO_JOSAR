...
const SUPABASE_CONFIG = {
    url: 'https://pqkjkeolvbfatagyljzf.supabase.co',
    key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBxa2prZW9sdmJmYXRhZ3lsanpmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg1NDc1MDMsImV4cCI6MjA5NDEyMzUwM30.uDNqCBZaQbLVsDlt5Lb_goyw2EWEB2DUNMDLJSG-Pvw'
};
...
function loginUser(username, password) {
    ...
    // Verificar usuarios cotizadores
    if (USUARIOS_COTIZADORES[username] && USUARIOS_COTIZADORES[username].password === password) {
        isUserLoggedIn = true;
        currentUsername = username;
        currentUserType = 'cotizador';
        localStorage.setItem('geoportal_user', username);
        localStorage.setItem('geoportal_user_type', 'cotizador');
        updateUIForLoggedInUser();
        return true;
    }
    return false;
...
async function exportarExcel(urb) {
    try {
        ...
        const camposSelect = 'lote,manzano,supm2,precio_lot,estado,vendedor,nom_cliente,cel_cliente,ci_cliente,precio_vent,monto_bs,tipo_venta';
...
document.getElementById('nuevo-cobro-form').addEventListener('submit', async (e) => {
    ...
    const adelantoPagado = parseFloat(loteSeleccionadoCobro.monto_reserva || 0);
